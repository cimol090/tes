  ...
  def onMessage(self, room, user, message):
   ...
   try:
    ...
    if message.body[0] in ["<"]:   
      ...
      ##BotStop
      if cmd =="dc":
         if user.name == "xenoryuu" or user.name == "rifky102":
...
#### Invite
    ...
    ###### Define            
      ...
      ##simi Lock
      if cmd == "mute" and self.getAccess(user) >= 3:
        if args:
          if args in simlock:
            room.message("_shhh")
          if args in self.roomnames:
        ...
        if not args:
          if room.name not in simlock:
             ...
             room.message(":|")
      ##Simi unlock
      if cmd == "unmute" and self.getAccess(user) >= 3:
        if args:
      ...
      if cmd == "lock":
        ...
        if args in locks:
          ...
          return
        if args in self.roomnames:
          if user.name in owners or user.name in admin or user.name in archknight:
            locks.append(args)
      ...
      if cmd == "unlock":
        ...
        if args in self.roomnames:
          if args in locks:
            if user.name in owners or user.name in admin or user.name in archwizard:
              locks.remove(args)
    ...
    ##### Whitelist
      elif cmd == "wl" and self.getAccess(user) >= 6:
        name = args
        name = getUname(room, name)
